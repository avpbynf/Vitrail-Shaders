 ...
 * <p>
 * Off unless the JVM is started with {@code -Dvitrail.passTimings=N}, N being the seconds between
 * two reports; a session that never sets it pays a static boolean per pass and nothing else. The
 * timestamps are written where the game's own Tracy profiler writes its zones, outside the pass on
 * the frame's command buffer, and they are written at the all-commands stage, so a pass's time is
