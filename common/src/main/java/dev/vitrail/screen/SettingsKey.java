 ...
 * <p>
 * The mapping and what a press does are here; registering it and asking it once a tick is each
 * loader's own business, because those are the two things they do differently. Asking on a tick
 * rather than on a key event is deliberate and is the same on both: the game only feeds key
 * mappings while no screen is open, so the shortcut cannot open a second copy of this screen over
 * the first, and it costs one boolean a tick.
 */
public final class SettingsKey {
	...
	/** Opens the screen if the key was pressed since the last tick, and does nothing otherwise. */
	public static void pollAndOpen() {
