 ...
 * chunk border or a hitbox is visible through the terrain in front of it. That clear lands on the
 * main target the whole engine reads, and it lands before {@code AfterLevel}, where the chain runs:
 * every frame the game has one gizmo to draw, the depth this engine hands the pack as
 * {@code depthtex0} is the far plane over the whole screen. Nothing about that shows as itself. The
 * fog, the depth of field and the ambient occlusion of the pack simply behave as though the player
 * were looking at an empty sky, which is a plausible picture and a false one.
 * <p>
